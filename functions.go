...
func createEmbed(ctx *discordflo.Context, s string) *discordgo.MessageEmbed {
	return &discordgo.MessageEmbed{Description: s, Color: 0x222a7f}
}
...
func GetAllUsers() (ctx *discordflo.Context, ms []*discordgo.User, err error) {
...
func GetUserByName(ctx *discordflo.Context, query string) (members []*discordgo.User, err error) {
...
func GuildGetUserByName(ctx *discordflo.Context, query, GuildID string) (members []*discordgo.User, err error) {
	MentionRegex := regexp.MustCompile(`<@!?(\d+)>`)
	var id, discrim string
	if MentionRegex.MatchString(query) {
		id = MentionRegex.FindStringSubmatch(query)[1]
...
func ParseTooManyUsers(ctx *discordflo.Context, query string, users []*discordgo.User) (*discordgo.Message, error) {
...
func GetUser(ctx *discordflo.Context, args ...string) (user *discordgo.User, err error) {
